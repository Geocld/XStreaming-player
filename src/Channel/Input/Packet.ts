...
export default class InputPacket {
    ...
    _writeGamepadData(packet:DataView, offset:number, frames:Array<InputFrame>){
        ...
        for (; frames.length > 0;) {
            ...
            if(shift !== undefined){
                ...
                const physicalPhysicality = Number.isFinite(inputWithPhysicality.PhysicalPhysicality)
                    ? (inputWithPhysicality.PhysicalPhysicality as number)
                    : this._calculateGamepadPhysicality(input)

                const virtualPhysicality = Number.isFinite(inputWithPhysicality.VirtualPhysicality)
    ...
    _calculateGamepadPhysicality(input: InputFrame, thumbstickDeadzone = 0) {
        ...
        if (input.DPadUp) { physicality |= GamepadInputPhysicality.DPadUp }
        if (input.DPadDown) { physicality |= GamepadInputPhysicality.DPadDown }
        if (input.DPadLeft) { physicality |= GamepadInputPhysicality.DPadLeft }
        if (input.DPadRight) { physicality |= GamepadInputPhysicality.DPadRight }
        if (input.Menu) { physicality |= GamepadInputPhysicality.Menu }
        if (input.View) { physicality |= GamepadInputPhysicality.View }
        if (input.LeftThumb) { physicality |= GamepadInputPhysicality.LeftThumb }
        if (input.RightThumb) { physicality |= GamepadInputPhysicality.RightThumb }
        if (input.LeftShoulder) { physicality |= GamepadInputPhysicality.LeftShoulder }
        if (input.RightShoulder) { physicality |= GamepadInputPhysicality.RightShoulder }
        if (input.Nexus) { physicality |= GamepadInputPhysicality.Nexus }
        if (input.A) { physicality |= GamepadInputPhysicality.A }
        if (input.B) { physicality |= GamepadInputPhysicality.B }
        if (input.X) { physicality |= GamepadInputPhysicality.X }
        if (input.Y) { physicality |= GamepadInputPhysicality.Y }
        if (input.LeftTrigger) { physicality |= GamepadInputPhysicality.LeftTrigger }
        if (input.RightTrigger) { physicality |= GamepadInputPhysicality.RightTrigger }
