...
export default class VideoComponent {
    ...
    create(srcObject) {
        ...
        if(videoHolder !== null){
            ...
            videoRender.addEventListener('pointercancel', (e) => this._client.getChannelProcessor('input')?.onPointerClick(e), { passive: false }),
            videoRender.addEventListener('touchstart', (e) => this._client.getChannelProcessor('input')?.onTouchEvent(e), { passive: false }),
            videoRender.addEventListener('touchmove', (e) => this._client.getChannelProcessor('input')?.onTouchEvent(e), { passive: false }),
            videoRender.addEventListener('touchend', (e) => this._client.getChannelProcessor('input')?.onTouchEvent(e), { passive: false }),
            videoRender.addEventListener('touchcancel', (e) => this._client.getChannelProcessor('input')?.onTouchEvent(e), { passive: false }),
    ...
    startFSR() {
        ...
        let DEFAULT_SHARPNESS = 0.2
        if (this._client._fsr_sharpness !== undefined) {
            DEFAULT_SHARPNESS = this._client._fsr_sharpness / 10
        }
        console.log('FSR sharpness:', DEFAULT_SHARPNESS)
        // dom size variables
        let width = 0
        let height = 0
        let aspect = 0
        ...
        aspect = this._video.videoWidth / this._video.videoHeight

        const video_format = this._client._video_format
        if (video_format && video_format.indexOf(':') !== -1) {
            const ratioParts = video_format.split(':')
            const ratioWidth = parseFloat(ratioParts[0])
            const ratioHeight = parseFloat(ratioParts[1])
            aspect = ratioWidth / ratioHeight
        }

        if (video_format === 'Stretch' || video_format === 'Zoom') {
